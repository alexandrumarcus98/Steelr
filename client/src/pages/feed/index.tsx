...
const Feed: React.FC = () => {
	...
	const [updatingFriendId, setUpdatingFriendId] = React.useState<string | null>(
		null,
	);
	...
	useEffect(() => {
		const timer = window.setTimeout(() => {
			...
			void dispatch(
				searchUsersThunk({ query: searchQuery, limit: 8, page: 1 }),
			)
				.unwrap()
				.then((result) => {
	...
	const handleToggleFriend = async (targetUser: UserSearchResult) => {
		try {
	...
	return (
		<div className="mx-auto max-w-3xl">
			...
			<section className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
				<div className="flex flex-col gap-1">
					...
					<p className="text-sm text-slate-500 dark:text-slate-400">
						...
						{user?.location?.country ? ` Your profile location: ${user.location.city ? `${user.location.city}, ` : ""}${user.location.country}.` : ""}
					</p>
				</div>
				...
				<div className="mt-4 space-y-3">
					{searchResults.map((candidate) => (
						<div
							key={candidate.id}
							className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950 sm:flex-row sm:items-center sm:justify-between"
						>
							<div>
							...
							<button
								...
								className="inline-flex items-center justify-center rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white transition-all duration-300 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-70 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
							>
			...
			{/* ── Sort tabs ───────────────────────────────────────────── */}
			<div className="mb-5 mt-4 flex border-b border-slate-200 dark:border-slate-800">
				{(["latest", "mostViewed"] as const).map((tab) => (
					...
					>
						...
						{sort === tab && (
							<span className="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-slate-900 dark:bg-slate-100" />
						)}
					</button>
				))}
			</div>
			...
			{status !== "loading" && items.length === 0 && !error && (
				<div className="rounded-2xl border border-slate-200 bg-white p-10 text-center dark:border-slate-800 dark:bg-slate-900">
					<p className="text-sm text-slate-500 dark:text-slate-400">No posts available yet.</p>
				</div>
			)}
			...
			<div className="space-y-3">
				{items.map((post) => (
					<article
						key={post.id}
						className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/30 dark:hover:border-slate-700 dark:hover:bg-slate-800"
					>
						{/* Author row */}
						<div className="mb-3 flex items-start justify-between gap-3">
							<div className="flex items-center gap-3">
								<div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white shadow-md dark:bg-slate-100 dark:text-slate-900">
						...
						<Link to={`/posts/${post.id}`} className="group/link block">
							{post.title && (
								<h2 className="mb-1.5 text-base font-semibold text-slate-900 transition-colors duration-200 group-hover/link:text-slate-700 dark:text-slate-100 dark:group-hover/link:text-slate-300">
									{post.title}
								</h2>
							)}
						...
						{/* Stats & like */}
						<div className="mt-4 flex items-center gap-5 border-t border-slate-200 pt-3.5 dark:border-slate-800">
							...
							<button
								...
								className={`ml-auto flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all duration-200 ${
									post.isLiked
										? "bg-slate-900/5 text-slate-900 ring-1 ring-slate-300 hover:bg-slate-900/10 dark:bg-slate-100/10 dark:text-slate-100 dark:ring-slate-700 dark:hover:bg-slate-100/15"
			...
			{hasMore && items.length > 0 && (
				<div className="flex justify-center pt-6">
					<button
						...
						className="rounded-full border border-slate-300 bg-white px-6 py-2.5 text-sm font-medium text-slate-700 transition-all duration-200 hover:border-slate-400 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-800"
					>
